...
class OrchidDataModule(VitalDataModule):
    """Implementation of the ``VitalDataModule`` for the ORCHID dataset."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if num_tab_attrs:
            modalities_shapes[OrchidTag.tabular_attrs] = (num_tab_attrs, 1)
...
@hydra.main(version_base=None, config_path="../../config/data", config_name="orchid")
def main(cfg: DictConfig):
    """Test loading a batch from the datamodule's train dataloader."""
    import hydra

    logger.info(f"Instantiating orchid datamodule based on the following Hydra config: \n{OmegaConf.to_yaml(cfg)}")
