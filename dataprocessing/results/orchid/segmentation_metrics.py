...
class SegmentationMetrics(Metrics):
        ...
        for label_tag, label in self.labels.items():
            ...
            for metric_tag, metric_fn in itertools.chain(self.scores.items(), self.distances.items()):
                ...
                if metric_tag in self.distances:
                    ...
                    metric_kwargs["voxelspacing"] = result.attrs[self.target_tag][OrchidTag.voxelspacing]
    ...
    def build_parser(cls) -> ArgumentParser:
        """Creates parser for orchid segmentation metrics processor.

        Returns:
            Parser object for orchid segmentation metrics processor.
        """
