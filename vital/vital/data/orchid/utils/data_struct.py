...
class Patient:
    ...
    def get_mask_attributes(self, mask_tag: str) -> Dict[ViewEnum, Dict[str, np.ndarray]]:
        ...
        return {
            ...
            if view != MISSING_TS_ATTRS else raise Exception(f"View '{view_enum}' not found for patient '{self.id}'.")
...
class View:
    ...
    def _is_mask_tag(self, data_tag: str) -> bool:
        ...
        return OrchidTag.mask in data_tag
    ...
    def add_image(
        ...
        if reference_img_tag:
            ...
            ref_voxelspacing = self.attrs[reference_img_tag][OrchidTag.voxelspacing]
        ...
        if precomputed_attrs is not None:
            ...
            self.attrs[tag] = {OrchidTag.voxelspacing: voxelspacing}
    ...
    def resize_image(
        ...
        if target_img_size:
            ...
            source_voxelspacing = self.attrs[source_img_tag][OrchidTag.voxelspacing]
            # For each dimension, the voxelspacing equals to `shape * voxelspacing / resized_shape`
            ...
            resized_img = resize_image_to_voxelspacing(
                ...
                self.attrs[source_img_tag][OrchidTag.voxelspacing],
                target_voxelspacing,
                resample=resample,
            )
        elif target_img_tag:
            ...
            target_voxelspacing = self.attrs[target_img_tag][OrchidTag.voxelspacing]
        else:
        ...
        attrs = {OrchidTag.voxelspacing: target_voxelspacing}
        if self._is_mask_tag(tag):
                ...
                if overlay_control_points:
                    ...
                    for struct_name, points_color in struct_colors.items():
                        ...
                        control_points = EchoMeasure.control_points(
                            ...
                            voxelspacing=seq_mask_attrs[OrchidTag.voxelspacing],
                        )
        ...
        if use_pil_backend:
            ...
            for tag, im_array in data_to_save.items():
                sitk_save(
                    ...
                    spacing=(*attrs_to_save[tag][OrchidTag.voxelspacing][::-1], 1),
