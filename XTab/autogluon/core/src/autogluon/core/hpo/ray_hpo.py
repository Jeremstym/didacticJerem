...
class RayTuneAdapter(ABC):
    ...
    def get_resources_per_trial(
        ...
        **kwargs,
        ...
        assert (
            isinstance(minimum_cpu_per_trial, int) and minimum_cpu_per_trial >= 1
        ...
        ), "Total num_gpus available must be greater or equal to minimum_gpu_per_trial"
...
def run(
    ...
    tuner = tune.Tuner(
        ...
        run_config=air.RunConfig(
            name=os.path.basename(save_dir), local_dir=os.path.dirname(save_dir), verbose=verbose, **run_config_kwargs
        ),
...
def _get_scheduler(hyperparameter_tune_kwargs: dict, supported_schedulers: Optional[List[str]] = None):
    ...
    if isinstance(scheduler, str):
        assert (
            scheduler in SCHEDULER_PRESETS
...
class AutommRayTuneLightningAdapter(RayTuneAdapter):
    ...
    def check_user_provided_resources_per_trial(self, resources_per_trial: Optional[dict] = None):
        if resources_per_trial is not None:
            ...
            logger.warning(
                "AutoMM does not support customized resources_per_trial. We will calculate it for you instead."
            )

    def get_resource_calculator(self, num_gpus):
        return ResourceCalculatorFactory.get_resource_calculator(
            calculator_type="ray_lightning_cpu" if num_gpus == 0 else "ray_lightning_gpu"
        )
    ...
    def trainable_args_update_method(self, trainable_args: dict) -> dict:
        ...
        trainable_args["hyperparameters"][
            "env.num_nodes"
        ] = 1  # num_nodes is not needed by ray lightning. Setting it to default, which is 1
