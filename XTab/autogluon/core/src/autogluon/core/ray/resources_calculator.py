...
class ResourceCalculator(ABC):
    ...
    def get_total_gpu_count(
        user_specified_num_gpus: int, model_default_num_gpus: int, num_gpus_available: Optional[float] = None
    ):
        if user_specified_num_gpus is not None:
            total_gpu = (
                get_gpu_count_all() if num_gpus_available is None else min(get_gpu_count_all(), num_gpus_available)
            )
    ...
    def get_total_cpu_count(
        user_specified_num_cpus: int, model_default_num_cpus: int, num_cpus_available: Optional[int] = None
    ):
    ...
    def get_resources_per_job(
        ...
        **kwargs,
        ...
        resources_per_job = get_tune_resources(
            num_workers=num_workers, num_cpus_per_worker=cpu_per_worker, use_gpu=False
        )
        batches = math.ceil(num_jobs / num_parallel_jobs)
...
class RayLightningGpuResourceCalculator(ResourceCalculator):
    ...
    def get_resources_per_job(
        ...
        **kwargs,
        ...
        resources_per_job = get_tune_resources(
            num_workers=num_workers, num_cpus_per_worker=cpu_per_worker, use_gpu=True
        )
        batches = math.ceil(num_jobs / num_parallel_jobs)
