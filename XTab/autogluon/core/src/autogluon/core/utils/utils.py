        ...
        if isinstance(self._splitter, RepeatedStratifiedKFold):
            ...
            with warning_filter():
                try:
                    ...
                    out = [
                        [train_index, test_index] for train_index, test_index in self._splitter.split(X_dummy, y_dummy)
                    ]
                    len_out = len(out)
            ...
            return [
                [train_index, test_index] for train_index, test_index in self._splitter.split(X, y, groups=self.groups)
            ]
...
def generate_train_test_split(
    X: DataFrame, y: Series, problem_type: str, test_size: float = 0.1, random_state=0, min_cls_count_train=1
    ...
    for feature in features:
        if isinstance(feature, tuple):
            ...
            if feature_name not in valid_features:
                raise ValueError(
                    f"Feature does not exist in data: {feature_name}\n" f"Valid Features:\n" f"{valid_features}"
                )
