...
def try_import_ray() -> ModuleType:
    ...
    try:
        ...
        if LooseVersion(ray.__version__) < LooseVersion(ray_min_version) or LooseVersion(
            ray.__version__
        ) >= LooseVersion(ray_max_version):
            msg = (
...
def try_import_ray_lightning():
    ...
    try:
        ...
        ray_lightning_version = pkg_resources.get_distribution(
            "ray_lightning"
        ).version  # ray_lightning doesn't have __version__...

        if not (
            LooseVersion(supported_ray_lightning_min_version)
        ...
        for (
            ray_lightning_versions,
            torch_lightning_versions,
        ) in ray_lightining_torch_lightning_compatibility_range_map.items():
            ...
            if (
                ...
                if not (
                    LooseVersion(torch_lightning_min_version)
...
def try_import_d8():
    try:
        ...
        raise ImportError(
            "`import d8` failed. d8 is an optional dependency.\n" "A quick tip is to install via `pip install d8`.\n"
        )
...
def try_import_autogluon_vision():
    try:
        ...
        raise ImportError(
            "`import autogluon.vision` failed.\n"
...
def try_import_vowpalwabbit():
    try:
        ...
        assert vowpalwabbit_version >= parse_version("8.10.1") and vowpalwabbit_version < parse_version(
            "8.11.0"
