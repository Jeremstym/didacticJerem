...
class Float(CheckType):
    ...
    def assert_valid(self, key: str, value):
        assert isinstance(value, numbers.Real), "{}: Value = {} must be of type float".format(key, value)
        assert (not self.lower) or value >= self.lower, "{}: Value = {} must be >= {}".format(key, value, self.lower)
        assert (not self.upper) or value <= self.upper, "{}: Value = {} must be <= {}".format(key, value, self.upper)
...
class Integer(CheckType):
    ...
    def assert_valid(self, key: str, value):
        assert isinstance(value, numbers.Integral), "{}: Value = {} must be of type int".format(key, value)
        assert (not self.lower) or value >= self.lower, "{}: Value = {} must be >= {}".format(key, value, self.lower)
        assert (not self.upper) or value <= self.upper, "{}: Value = {} must be <= {}".format(key, value, self.upper)
...
class Categorical(CheckType):
    ...
    def assert_valid(self, key: str, value):
        assert isinstance(value, str) and value in self.choices, "{}: Value = {} must be in {}".format(
            key, value, self.choices
        )
...
class Boolean(CheckType):
    def assert_valid(self, key: str, value):
        assert isinstance(value, bool), "{}: Value = {} must be boolean".format(key, value)


def check_and_merge_defaults(
    options: dict, mandatory: Set[str], default_options: dict, constraints: Dict[str, CheckType] = None, dict_name=None
) -> dict:
    """
