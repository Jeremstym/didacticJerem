...
from .utils import (get_available_disk_size, get_cpu_count, get_gpu_count_all,
                    get_gpu_free_memory, get_memory_size)
...
def _get_autogluon_versions():
    ...
    for pkg in list(pkgutil.iter_modules(autogluon.__path__, autogluon.__name__ + ".")):
        if (
            pkg.name == "autogluon.version"
        ):  # autogluon.version will be recognized as a submodule by pkgutil. We don't need it
            continue
        try:
