...
class Scorer(object, metaclass=ABCMeta):
    ...
    def add_alias(self, alias):
        if alias == self.name:
            raise ValueError(
                f'The alias "{alias}" is the same as the original name "{self.name}". ' f"This is not allowed."
            )
...
def _add_scorer_to_metric_dict(metric_dict, scorer):
    ...
    for alias in scorer.alias:
        if alias in metric_dict:
            raise ValueError(
                f"Duplicated alias found! scorer={scorer}, alias={alias}. " f"Consider to use a different alias."
            )
...
roc_auc = make_scorer("roc_auc", sklearn.metrics.roc_auc_score, greater_is_better=True, needs_threshold=True)
