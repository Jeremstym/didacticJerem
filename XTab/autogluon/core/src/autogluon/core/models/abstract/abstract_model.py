...
from ... import Space, metrics
from ...constants import (AG_ARGS_FIT, BINARY, OBJECTIVES_TO_NORMALIZE,
                          QUANTILE, REFIT_FULL_SUFFIX, REGRESSION)
...
from ...utils import (compute_permutation_feature_importance,
                      compute_weighted_metric, get_cpu_count,
                      get_pred_from_proba, infer_eval_metric,
                      infer_problem_type, normalize_pred_probas)
from ...utils.exceptions import (NotEnoughMemoryError, NoValidFeatures,
                                 TimeLimitExceeded)
...
class AbstractModel:
    ...
    def _validate_fit_memory_usage(self, **kwargs):
        ...
        if ratio > (0.9 * max_memory_usage_ratio):
            logger.warning(
                "\tWarning: Not enough memory to safely train model, roughly requires: %s GB, but only %s GB is available..."
                % (round(approx_mem_size_req / 1e9, 3), round(available_mem / 1e9, 3))
            )
            raise NotEnoughMemoryError
        elif ratio > (0.6 * max_memory_usage_ratio):
            logger.warning(
                "\tWarning: Potentially not enough memory to safely train model, roughly requires: %s GB, but only %s GB is available..."
                % (round(approx_mem_size_req / 1e9, 3), round(available_mem / 1e9, 3))
            )
