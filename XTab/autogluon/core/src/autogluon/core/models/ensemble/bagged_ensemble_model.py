...
from ...constants import (MULTICLASS, QUANTILE, REFIT_FULL_SUFFIX, REGRESSION,
                          SOFTCLASS)
...
from .fold_fitting_strategy import (AbstractFoldFittingStrategy,
    ...
    **kwargs
    ...
    def _fit(
        ...
        **kwargs,
        ...
        if (not self._get_tags_child().get("can_refit_full", False) or k_fold == 1) and not self.params.get(
            "save_bag_folds", True
        ):
    ...
    def _get_default_fold_fitting_strategy(self):
        ...
        if fold_fitting_strategy == "sequential_local":
            ...
            try:
                ...
                warning_msg = (
                    f"Will use sequential fold fitting strategy because import of ray failed. Reason: {str(e)}"
                )
                dup_filter.attach_filter_targets(warning_msg)
                logger.warning(warning_msg)
    ...
    def _fit_folds(
        ...
        **kwargs,
        ...
        if (
            type(fold_fitting_strategy) == ParallelLocalFoldFittingStrategy
            and not fold_fitting_strategy.is_mem_sufficient()
        ):
    ...
    def compute_feature_importance(
        self, X, y, features=None, silent=False, time_limit=None, is_oof=False, **kwargs
    ) -> pd.DataFrame:
        if features is None:
    ...
    def _hyperparameter_tune(self, X, y, X_val, y_val, k_fold, hpo_executor, **kwargs):
        time_start = time.time()
        ...
        num_folds_in_parallel = min(
            num_folds_in_parallel_with_mem, num_folds_in_parallel_with_cpu, num_folds_in_parallel_with_gpu
        )
