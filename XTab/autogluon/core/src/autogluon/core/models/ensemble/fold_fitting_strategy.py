...
from ...utils.exceptions import (NotEnoughCudaMemoryError,
                                 NotEnoughMemoryError, TimeLimitExceeded)
...
class LocalFoldFittingStrategy(AbstractFoldFittingStrategy):
    ...
    def _get_fold_properties(fold_ctx):
        fold, folds_finished, folds_left, folds_to_fit, is_last_fold, model_name_suffix = [
            fold_ctx[f]
            for f in ["fold", "folds_finished", "folds_left", "folds_to_fit", "is_last_fold", "model_name_suffix"]
        ]
...
class SequentialLocalFoldFittingStrategy(LocalFoldFittingStrategy):
    ...
    def _fit(self, model_base, time_start_fold, time_limit_fold, fold_ctx, kwargs):
        ...
        fold_model.fit(
            X=X_fold, y=y_fold, X_val=X_val_fold, y_val=y_val_fold, time_limit=time_limit_fold, **kwargs_fold
        )
...
def _ray_fit(
    ...
    (
        ...
        is_last_fold,
        model_name_suffix,
    ) = LocalFoldFittingStrategy._get_fold_properties(fold_ctx)
    train_index, val_index = fold
    fold_model = copy.deepcopy(model_base)
    fold_model.name = f"{fold_model.name}{model_name_suffix}"
    ...
    fold_model, pred_proba = _ray_predict_oof(
        fold_model, X_val_fold, y_val_fold, time_train_end_fold, resources["num_cpus"], save_bag_folds
    )
    fold_model.save()
    return (
        fold_model.name,
...
class ParallelLocalFoldFittingStrategy(LocalFoldFittingStrategy):
    ...
    def after_all_folds_scheduled(self):
        ...
        while unfinished:
            ...
            finished = finished[0]
            try:
                fold_model, pred_proba, time_start_fit, time_end_fit, predict_time, predict_1_time = self.ray.get(
                    finished
                )
    ...
    def _fit(
        self, model_base_ref, X_ref, y_ref, X_pseudo_ref, y_pseudo_ref, time_limit_fold, fold_ctx, resources, kwargs
    ):
        fold, folds_finished, folds_left, folds_to_fit, is_last_fold, model_name_suffix = self._get_fold_properties(
            fold_ctx
        )
        ...
        if self.sample_weight is not None:
            if is_pseudo:
                ...
                kwargs_fold["sample_weight"] = self.sample_weight[train_index]
                kwargs_fold["sample_weight_val"] = self.sample_weight[val_index]
        return self._ray_fit.options(**resources).remote(
            model_base_ref,
            self.bagged_ensemble_model.path,
            X_ref,
            y_ref,
            X_pseudo_ref,
            y_pseudo_ref,
    ...
    def _update_bagged_ensemble(
        self, fold_model, pred_proba, time_start_fit, time_end_fit, predict_time, predict_1_time, fold_ctx
    ):
        _, val_index = fold_ctx["fold"]
