...
from ..augmentation.distill_utils import (augment_data,
                                          format_distillation_labels)
...
from ..constants import (AG_ARGS, BINARY, MULTICLASS, QUANTILE,
                         REFIT_FULL_NAME, REFIT_FULL_SUFFIX, REGRESSION)
from ..data.label_cleaner import LabelCleanerMulticlassToBinary
from ..models import (AbstractModel, BaggedEnsembleModel,
                      GreedyWeightedEnsembleModel, SimpleWeightedEnsembleModel,
                      StackerEnsembleModel, WeightedEnsembleModel)
from ..utils import (compute_permutation_feature_importance,
                     compute_weighted_metric, default_holdout_frac,
                     extract_column, generate_train_test_split,
                     get_pred_from_proba, infer_eval_metric)
from ..utils.exceptions import (NoGPUError, NotEnoughCudaMemoryError,
                                NotEnoughMemoryError, NoValidFeatures,
                                TimeLimitExceeded)
...
class AbstractTrainer:
    ...
    def __init__(
        ...
        logger.log(
            25, f"AutoGluon will gauge predictive performance using evaluation metric: '{self.eval_metric.name}'"
        )
        ...
        self._time_limit = (
            None  # Internal float of the total time limit allowed for a given fit call. Used in logging statements.
        )
        self._time_train_start = (
            None  # Internal timestamp of the time training started for a given fit call. Used in logging statements.
        )
        ...
        self._regress_preds_asprobas = (
            False  # whether to treat regression predictions as class-probabilities (during distillation)
        )
    ...
    def stack_new_level_core(
        ...
        **kwargs,
        ...
        base_model_names = self._filter_base_models_via_infer_limit(
            base_model_names=base_model_names, infer_limit=infer_limit, infer_limit_modifier=0.8
        )
    ...
    def get_pred_proba_from_model(self, model, X, model_pred_proba_dict=None, cascade=False):
        ...
        model_pred_proba_dict = self.get_model_pred_proba_dict(
            X=X, models=models, model_pred_proba_dict=model_pred_proba_dict, cascade=cascade
        )
    ...
    def _construct_model_pred_order_with_pred_dict(
        self, models: List[str], models_to_ignore: List[str] = None
    ) -> List[str]:
        """
        Constructs a list of model names in order of inference calls required to infer on all the models.
    ...
    def get_inputs_to_stacker(
        self, X, base_models, model_pred_proba_dict=None, fit=False, use_orig_features=True, use_val_cache=False
    ):
        ...
        if fit:
            ...
            for level in range(level_start, level_end):
                ...
                X = dummy_stackers[level + 1].preprocess(
                    X=X, preprocess_nonadaptive=False, fit=False, compute_base_preds=True
                )
        ...
        for level in levels:
            ...
            for model in models_level:
                ...
                if reuse_first_fold:
                    ...
                    model_full._user_params_aux["max_memory_usage_ratio"] = (
                        model.params_aux["max_memory_usage_ratio"] * 1.15
                    )
    ...
    def refit_ensemble_full(self, model="all") -> dict:
        ...
        for model in ensemble_set:
            if model in model_full_dict and model_full_dict[model] in existing_models:
                logger.log(
                    20,
                    f"Model '{model}' already has a refit _FULL model: '{model_full_dict[model]}', skipping refit...",
                )
            else:
    ...
    def get_model_best(self, can_infer=None, allow_full=True, infer_limit=None):
        ...
        models_predict_time = self.get_models_attribute_full(
            models=models, attribute="predict_time"
        )  # FIXME: Refit_full???
    ...
    def _train_single(
        self, X, y, model: AbstractModel, X_val=None, y_val=None, pretrain_kwargs=None, **model_fit_kwargs
    ) -> AbstractModel:
        """
        Trains model but does not add the trained model to this Trainer.
        Returns trained model object.
        """
        model = model.fit(X=X, y=y, X_val=X_val, y_val=y_val, pretrain_kwargs=pretrain_kwargs, **model_fit_kwargs)
        return model
        ...
        **model_fit_kwargs,
        ...
        try:
            ...
            if (
                ...
                logger.log(
                    15, f"{len(X_pseudo)} extra rows of pseudolabeled data added to training set for {model.name}"
                )
                model = self._train_single(X_w_pseudo, y_w_pseudo, model, X_val, y_val, **model_fit_kwargs)
            else:
                model = self._train_single(
                    X, y, model, X_val, y_val, pretrain_kwargs=pretrain_kwargs, **model_fit_kwargs
                )
    ...
    def _predict_model(self, X, model, model_pred_proba_dict=None, cascade=False):
        y_pred_proba = self._predict_proba_model(
            X=X, model=model, model_pred_proba_dict=model_pred_proba_dict, cascade=cascade
        )
    ...
    def _predict_proba_model(self, X, model, model_pred_proba_dict=None, cascade=False):
        return self.get_pred_proba_from_model(
            model=model, X=X, model_pred_proba_dict=model_pred_proba_dict, cascade=cascade
        )
    ...
    def _get_feature_prune_proxy_model(
        self, proxy_model_class: Union[AbstractModel, None], level: int
    ) -> AbstractModel:
        """
