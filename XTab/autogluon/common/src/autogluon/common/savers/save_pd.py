...
def save(
    path, df, index=False, verbose=True, type=None, sep=",", compression="gzip", header=True, json_dump_columns=None
):
...
def save_multipart(
    ...
    multiprocessing_utils.execute_multiprocessing(
        workers_count=workers_count, transformer=save_multipart_child, chunks=full_chunks
    )
