...
def test_get_validated_path_with_compression_fn():
    ...
    assert (
        compression_utils.get_validated_path(no_extension_filepath, compression_fns[0]) == expected_no_ext_gzip_filepath
    )
    assert (
        compression_utils.get_validated_path(no_extension_filepath, compression_fns[1]) == expected_no_ext_lzma_filepath
    )
    assert (
    ...
    assert (
        compression_utils.get_validated_path(single_extension_filepath, compression_fns[2]) == expected_pkl_bz2_filepath
    )
