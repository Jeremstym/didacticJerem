...
class DatetimeFeatureGenerator(AbstractFeatureGenerator):
    ...
    def _generate_features_datetime(self, X: DataFrame) -> DataFrame:
        ...
        for datetime_feature in self.features_in:
            ...
            X_datetime[datetime_feature] = pd.to_datetime(X[datetime_feature], errors="coerce").fillna(
                self._fillna_map[datetime_feature]
            )
            ...
            for feature in self.features:
                X_datetime[datetime_feature + "." + feature] = getattr(X_datetime[datetime_feature].dt, feature).astype(
                    int
                )
