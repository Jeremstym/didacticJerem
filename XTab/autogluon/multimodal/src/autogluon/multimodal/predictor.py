...
from .constants import (AUTOMM, AUTOMM_TUTORIAL_MODE, BBOX, BEST,
                        BEST_K_MODELS_FILE, BINARY, CLASSIFICATION,
                        COLUMN_FEATURES, DATA, DEEPSPEED_MIN_PL_VERSION,
                        DEEPSPEED_MODULE, DEEPSPEED_OFFLOADING,
                        DEEPSPEED_STRATEGY, DEPRECATED_ZERO_SHOT,
                        FEATURE_EXTRACTION, FEATURES, FEW_SHOT, GREEDY_SOUP,
                        LABEL, LAST_CHECKPOINT, LOGITS, MAP, MASKS, MAX, MIN,
                        MODEL, MODEL_CHECKPOINT, MULTICLASS, NER, NER_RET,
                        OBJECT_DETECTION, OCR_TEXT_DETECTION,
                        OCR_TEXT_RECOGNITION, PROBABILITY, RAY_TUNE_CHECKPOINT,
                        REGRESSION, SCORE, TEXT, UNIFORM_SOUP, Y_PRED,
                        Y_PRED_PROB, Y_TRUE, ZERO_SHOT_IMAGE_CLASSIFICATION)
from .data.datamodule import BaseDataModule
from .data.infer_types import (
    infer_column_types, infer_label_column_type_by_problem_type_and_pipeline,
    infer_problem_type_output_shape, infer_rois_column_type)
from .data.preprocess_dataframe import MultiModalFeaturePreprocessor
from .data.utils import (apply_data_processor, apply_df_preprocessor,
                         get_collate_fn, get_per_sample_features)
...
from .utils import (AutoMMModelCheckpoint, AutoMMModelCheckpointIO,
                    COCODataset, CustomUnpickler, DDPCacheWriter, LogFilter,
                    apply_log_filter, assign_feature_column_names,
                    average_checkpoints, cocoeval,
                    compute_inference_batch_size, compute_num_gpus,
                    compute_score, create_fusion_data_processors,
                    create_fusion_model, data_to_df, extract_from_output,
                    filter_search_space, from_coco, from_coco_or_voc,
                    get_config, get_detection_classes,
                    get_local_pretrained_config_paths, get_minmax_mode,
                    get_mixup, get_onnx_input, getCOCOCatIDs, hpo_trial,
                    infer_batch, infer_dtypes_by_model_names, infer_metrics,
                    infer_precision, infer_scarcity_mode_by_data_size,
                    init_df_preprocessor, init_pretrained,
                    load_text_tokenizers, logits_to_prob,
                    modify_duplicate_model_names, process_save_path,
                    save_pretrained_model_configs, save_text_tokenizers,
                    select_model, tensor_to_ndarray, try_to_infer_pos_label,
                    turn_on_off_feature_column_info, update_config_by_rules,
                    update_tabular_config_by_resources, use_realtime)
...
class MultiModalPredictor:
    ...
    def _hyperparameter_tune(self, hyperparameter_tune_kwargs, resources, **_fit_args):
        from autogluon.core.hpo.ray_hpo import (AutommRayTuneAdapter,
    ...
    def _setup_distillation(
        ...
        output_feature_adaptor = (
            nn.Linear(student_model_dim, teacher_model_dim) if teacher_model_dim != student_model_dim else nn.Identity()
        )
            ...
            try:
                if finetune_on is not None:
                    ...
                    if pretrain_kwargs is not None and "bucket_name" in pretrain_kwargs:
                        ...
                        s3.Bucket(pretrain_kwargs["bucket_name"]).download_file(foundation_model, "./pretrained.ckpt")
        ...
        with warnings.catch_warnings():
            ...
            if pretrain_kwargs is not None and not pretrain_kwargs["is_pretrain"]:
                ...
                while True:
                    try:
                        target = (
                            pretrain_kwargs["folder_name"]
                            + "/iter_"
                            + str(-1)
                            + "/"
                            + pretrain_kwargs["name"]
                            + ".ckpt"
                        )
                        checkpoint = {
                            "state_dict": {
                                name: param for name, param in self._model.fusion_transformer.state_dict().items()
                            }
                        }
